...
import { startTemplate, stopTemplate, resetTemplate } from "@campshell/core";
  ...
  await stopTemplate(
    ...
    },
    {
      onStopped: ({ templateName }) => {
        process.stderr.write(
          `✓ Journal stopped. Data preserved at ${path.join(config.home, "data", templateName)}\n`,
        );
      },
    },
  );
...
export async function reset(config: LifecycleConfig): Promise<void> {
  if (!config.skipPrompt) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question(
        "This will delete all Journal data. Continue? [y/N] ",
        resolve,
      );
    });
    rl.close();
  ...
  await resetTemplate(
    ...
    {
      onReset: () => {
        process.stderr.write("✓ Journal data reset to defaults\n");
      },
    },
  );
}
