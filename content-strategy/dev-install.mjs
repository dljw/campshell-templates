/**
 * Dev install script — builds and installs the Content Strategy template into
 * ~/.campshell/templates/ so it's picked up by `pnpm tauri:dev`.
 *
 * Usage (from campshell-templates/content-strategy/):
...
console.log(`\n✓ Content Strategy template installed (${result.name} v${result.version})`);
