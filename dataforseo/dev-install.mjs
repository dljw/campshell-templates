/**
 * Dev install script — builds and installs the DataForSEO template into
 * ~/.campshell/templates/ so it's picked up by `pnpm tauri:dev`.
 *
 * Usage (from campshell-templates/dataforseo/):
...
console.log(`\n✓ DataForSEO template installed (${result.name} v${result.version})`);
