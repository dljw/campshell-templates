...
import type { SecretsStatusResponse } from "../hooks/useDataForSeo.js";
...
export function SettingsView({ secretsStatus, onSetSecrets }: SettingsViewProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <Card>
        <CardHeader>
          <CardTitle>DataForSEO Configuration</CardTitle>
          <CardDescription>
            Configure your API credentials to enable DataForSEO operations.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {saveSuccess && (
            <div className="bg-green-500/10 text-green-700 border border-green-200 p-4 rounded-md text-sm font-medium">
              ✓ Credentials saved successfully. You can now use all DataForSEO operations.
            </div>
          )}

          {!secretsStatus.allRequiredConfigured && !saveSuccess && (
            <div className="bg-yellow-500/10 text-yellow-600 p-4 rounded-md text-sm">
              Enter your API credentials below to get started.
            </div>
          )}
