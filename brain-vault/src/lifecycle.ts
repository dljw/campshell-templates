...
export async function reset(config: LifecycleConfig): Promise<void> {
  if (!config.skipPrompt) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question("This will delete all Brain Vault data. Continue? [y/N] ", resolve);
    });
    rl.close();
  ...
  await resetTemplate(
    ...
    {
      onReset: () => {
        process.stderr.write("✓ Brain Vault data reset to defaults\n");
      },
    },
  );
}
