...
export async function stop(config: LifecycleConfig): Promise<void> {
  await stopTemplate(
    {
      home: config.home,
      templateName: "seo-tracker",
      skipDashboard: config.skipDashboard,
    },
    {
      onStopped: ({ templateName }) => {
        process.stderr.write(
          `✓ SEO Tracker stopped. Data preserved at ${path.join(config.home, "data", templateName)}\n`,
        );
      },
    },
  );
...
export async function reset(config: LifecycleConfig): Promise<void> {
  if (!config.skipPrompt) {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question("This will delete all SEO Tracker data. Continue? [y/N] ", resolve);
    });
    rl.close();
  ...
  await resetTemplate(
    ...
    {
      onReset: () => {
        process.stderr.write("✓ SEO Tracker data reset to defaults\n");
      },
    },
  );
}
