/**
 * Dev install script — builds and installs the SEO Tracker template into
 * ~/.campshell/templates/ so it's picked up by `pnpm tauri:dev`.
 *
 * Usage (from campshell-templates/seo-tracker/):
...
console.log(`\n✓ SEO Tracker template installed (${result.name} v${result.version})`);
