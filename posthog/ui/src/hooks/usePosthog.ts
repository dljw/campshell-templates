...
export function usePosthog(apiBase = ""): UsePosthogReturn {
  ...
  const fetchSecretsStatus = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase}/api/services/posthog/secrets/status`);
      if (res.ok) {
  ...
  const fetchRuns = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase}/api/services/posthog/runs`);
      if (res.ok) {
        const data = await res.json();
        setRuns(data.runs || []);
      }
  ...
  const setSecrets = useCallback(
    async (secrets: Record<string, string>) => {
      try {
        const res = await fetch(`${apiBase}/api/services/posthog/secrets`, {
          method: "POST",
  ...
  const executeOperation = useCallback(
    async (operation: string, input: unknown) => {
      setIsExecuting(true);
      try {
        const res = await fetch(`${apiBase}/api/services/posthog/${operation}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
