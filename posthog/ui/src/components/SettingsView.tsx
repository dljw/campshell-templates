...
import type { SecretsStatusResponse } from "../hooks/usePosthog.js";
    ...
    <div className="max-w-2xl mx-auto py-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure your PostHog API credentials. The Project API Key powers event capture and
            feature flag evaluation. The Personal API Key + Project ID are only required for
            the list-events operation.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {!secretsStatus.allRequiredConfigured && !saveSuccess && (
            <div className="bg-yellow-500/10 text-yellow-600 p-4 rounded-md text-sm">
              Enter your Project API Key below to get started.
            </div>
          )}
            ...
            {secretsStatus.secrets.map((secret) => (
              <div key={secret.key} className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    id={secret.key}
                    type={secret.key === "POSTHOG_INSTANCE" || secret.key === "POSTHOG_PROJECT_ID" ? "text" : "password"}
