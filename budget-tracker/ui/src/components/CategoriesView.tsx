...
import type { Category, CategoryColor } from "../types.js";
...
export function CategoriesView({ categories, isLoading, onUpdate }: CategoriesViewProps) {
  ...
  function handleAdd() {
    ...
    const newCategory: Category = {
      id: generateId(newName) || `cat-${Date.now().toString(36)}`,
          ...
          {categories.map((category) => (
            <Card key={category.id} className="relative group hover:bg-muted/20 transition-colors">
              <CardContent className="flex items-center gap-3 p-4">
                <Badge
                  variant="outline"
                  className={`capitalize ${category.color ? (COLOR_CLASS[category.color as CategoryColor] ?? "") : ""}`}
                >
                  {category.color ?? "—"}
                </Badge>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>Color</Label>
              <Select value={newColor} onValueChange={(v) => setNewColor(v as CategoryColor)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
